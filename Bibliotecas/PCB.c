...
{												//esta hecho asi para permitir detectar errores y actuar acorde a ellos
	...
	if((nbytes = recv(socket, scriptPathInFS, scriptLength, MSG_WAITALL)) <= 0)
	{
		return nbytes;
	}
	else
	...
	if((nbytes = recv(socket, &totalInstructionsExecuted, sizeof(uint32_t), MSG_WAITALL)) <= 0)
	{
		return nbytes;
	}
	else
	...
	if((nbytes = recv(socket, &instructionsExecutedOnLastExecution, sizeof(uint32_t), MSG_WAITALL)) <= 0)
	{
		return nbytes;
	}
	else
	...
	if((nbytes = recv(socket, &instructionsUntilIoOrEnd, sizeof(uint32_t), MSG_WAITALL)) <= 0)
	{
		return nbytes;
	}
	else
